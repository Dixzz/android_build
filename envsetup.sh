...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^pearl_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $PEARL_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        PEARL_BUILD=
    fi
    export PEARL_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the PearlOS GitHub
        T=$(gettop)
        cd $T > /dev/null
